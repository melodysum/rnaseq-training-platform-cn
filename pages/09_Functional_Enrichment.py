...
ORA 使用 Fisher 精确检验，问的是：
> *我的 DE 基因里，某个通路的基因出现的比例，是否比随机期望的多？*

**使用真实数据库时为什么可能没有显著结果**

用 MSigDB Hallmark 或 C2:CP 搭配 demo 数据时：

- Demo 数据共有 **1,038 个显著 DE 基因**（padj < 0.05，|log2FC| >= 1）
- 其中只有约 **246 个基因**能与 Hallmark 通路中的基因匹配
- 这 246 个匹配基因**分散在多条通路中**，没有任何一条通路的 overlap 足够集中，无法达到统计显著
- 经过 BH FDR 校正后，所有通路的校正 p 值均超过 0.05

**根本原因**

Demo 数据是**随机模拟**生成的，其 DE 基因不代表任何真实生物学过程，因此不会集中落在任何真实生物学通路中。MSigDB 不会在没有信号的情况下强行给出显著结果。

**使用真实数据时的预期结果**

上传真实实验数据（例如结核病数据集 GSE167232）后，HALLMARK_INTERFERON_GAMMA_RESPONSE 或 REACTOME_INTERFERON_SIGNALING 等通路应会显示出清晰、显著的富集结果。
            """)
